...
class ParticleSystem{
  ...
  void addParticle(PVector pos, PVector force, float r, float g, float b){
    Particle p = new Particle(pos, force, r, g, b, random(0.5, 5), 100);
  ...
  void draw(PImage img){
    ...
    for (int w=0; w<img.width; w+=grid_size) {
      for (int h=0; h<img.height; h+=grid_size) {
        ...
        if(aveFlow.mag() > treshold && this.particles.size() < maxN) {
          this.addParticle(position, aveFlow.mult(0.8), red(img.pixels[loc]), green(img.pixels[loc]), blue(img.pixels[loc]));
        }
      }
    }
